...
interface KExpectable<T>: Expectable<T> {
    ...
    override infix fun shouldHave(expectation: ExpectedToHave<T>): OnValueChain<T, ExpectedToHave<T>>  = verify(expectation)

    infix fun shouldHave(expectations: Array<ExpectedToHave<T>>) { verify(expectations) }

    override infix fun shouldMatch(expectation: ExpectedToMatch<T>): OnValueChain<T, ExpectedToMatch<T>> = verify(expectation)
